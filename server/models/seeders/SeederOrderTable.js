...
fake_order_data.forEach(row => {
    ...
    values.push([row.order_number, row.order_date, row.order_status, row.products_list, row.total, row.address, row.size, row.id_supplier_product, row.id_consumer, row.id_vehicle])     
});
