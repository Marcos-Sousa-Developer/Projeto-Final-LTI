...
const statement = "INSERT INTO suppliers (uid, name, email, nif, mobile_number, address, status) VALUES ? "

let values = []

fake_supplier_data.forEach(row => {

    values.push([row.uid,row.name, row.email, row.nif, row.mobile_number, row.address, row.status])     
});
